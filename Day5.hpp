...
// == Part 1 ==================================================================================
{
	...
	{	std::ifstream ifs("input_day5.txt");
		int num;
	...
	for(;;)
	{
		const int jmpOffset = input[currentIndex];

		++input[currentIndex];

		currentIndex += jmpOffset;
		++steps;
		
		if(currentIndex < 0 || currentIndex >= input.size()) break;
	}
	std::cout << steps <<  " steps needed for part a!" << std::endl;
}
// == Part 2 ==================================================================================
{
	std::vector<int> input;
	...
	int steps = 0, currentIndex = 0;
	for(;;)
	{
		const int jmpOffset = input[currentIndex];

		if(jmpOffset >= 3) --input[currentIndex];
		else ++input[currentIndex];
	...
	std::cout << steps <<  " steps needed for part b!" << std::endl;
}
}
